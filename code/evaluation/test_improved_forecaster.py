...
c = loader.load_all()
forecaster = ImprovedForecaster(c.rate_table)
ranker = PlanRanker()
...
for req in c.sample_requests:
    ...
    exp_status = str(row['affordability_status'])
    exp_method = str(row['recommended_payment_method'])
    exp_plan = str(row['payment_plan'])
    exp_earliest = str(row['earliest_date_for_full_payment']) if pd.notna(row['earliest_date_for_full_payment']) else ''
    exp_sc = str(row['spending_changes_needed'])
    exp_safe = str(row['amount_safe_to_pay'])

    gen_status = trace.affordability_status
    gen_method = trace.recommended_payment_method
    gen_plan = trace.payment_plan
    gen_earliest = trace.earliest_date_for_full_payment.isoformat() if trace.earliest_date_for_full_payment else ''
    gen_sc = trace.spending_changes_needed
    gen_safe = str(trace.amount_safe_to_pay)

    diffs = {}
    if gen_status != exp_status: diffs['status'] = (gen_status, exp_status)
    if gen_method != exp_method: diffs['method'] = (gen_method, exp_method)
    if gen_plan != exp_plan: diffs['plan'] = (gen_plan, exp_plan)
    if gen_earliest != exp_earliest: diffs['earliest'] = (gen_earliest, exp_earliest)
    if gen_sc != exp_sc: diffs['sc'] = (gen_sc, exp_sc)
    if abs(Decimal(gen_safe) - Decimal(exp_safe)) > Decimal("0.05"): diffs['safe'] = (gen_safe, exp_safe)

    if not diffs:
        exact += 1
        print(f"MATCH: {req.request_id}")
    else:
        print(f"DIFF: {req.request_id} -> {diffs}")

print(f"\nEXACT MATCHES: {exact}/25")
